      ...
      footer={
        <div className="flex items-center justify-end gap-2">
          ...
          <button
            ...
            className="rounded-md px-5 py-2 text-sm font-medium text-paper-50 bg-accent hover:bg-accent-hover disabled:opacity-50 transition-colors shadow-soft"
          >
            保存顺序
          </button>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[14px] font-medium text-ink-900 truncate">
                    {it.name?.trim() ? it.name : <span className="text-ink-400 italic">未填写品名</span>}
                  </div>
                  <div className="text-[11.5px] text-ink-500 truncate mt-0.5">
                    {it.spec?.trim() ? it.spec : <span className="text-ink-400 italic">无规格</span>}
                  </div>
                  {it.remark?.trim() && (
                ...
                <div className="shrink-0 flex flex-col gap-1">
                  <button
                    ...
                    disabled={idx === 0}
                    className="w-8 h-8 grid place-items-center rounded-md border border-line bg-paper-50 text-ink-600 hover:bg-paper-200 disabled:opacity-30 transition-colors"
                    aria-label="上移"
                    title="上移"
                  >
                    ▲
